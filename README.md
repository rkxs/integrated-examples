**这里是主流科学上网的优化配置及最优组合示例。如是不太了解科学上网，建议先依次从简单到复杂参考及部署。示例实现了：端口转发到进程转发、端口回落到进程回落、端口分流到进程分流及PROXY protocol的应用实现。**  
注：  
1、端口转发、端口回落、端口分流指基于local loopback的应用，实现的不同业务功能。  
