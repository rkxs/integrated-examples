...
4、因 trojan(trojan-go) 不支持 PROXY protocol（接收），而 nginx SNI 中的 PROXY protocol 发送是针对共用端口全局模式，故所有配置不启用此项应用。

5、因 trojan(trojan-go) 不支持 Unix Domain Socket，故 trojan(trojan-go) 全部端口回落，nginx SNI 对 trojan(trojan-go) 仅端口分流。
