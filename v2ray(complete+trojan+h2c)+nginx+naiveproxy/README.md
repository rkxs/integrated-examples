...
6、v2ray SNI 分流不支持 PROXY protocol（发送），故配置1：没有启用 PROXY protocol，端口回落；配置2：没有启用 PROXY protocol，进程回落。
...
8、配置3：没有启用 PROXY protocol，端口回落。配置4：没有启用 PROXY protocol，进程回落。配置5：启用了 PROXY protocol，进程回落。
