...
5、v2ray SNI 分流不支持 PROXY protocol（发送），故全部配置不启用此项应用。

6、配置4：端口转发、端口回落\分流及 v2ray SNI 的端口分流，没有启用 PROXY protocol。配置5：进程转发、进程回落\分流及 v2ray SNI 的进程分流，没有启用 PROXY protocol。
